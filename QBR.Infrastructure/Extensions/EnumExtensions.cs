...
namespace QBR.Infrastructure.Extensions
{
    public static class EnumExtensions
    {
        ...
        /// </summary>
        public static IEnumerable<string> GetAllDescriptions<TEnum>() where TEnum : struct, IConvertible, IComparable, IFormattable
        {
            if (!typeof(TEnum).IsEnum)
                throw new ArgumentException("TEnum must be an Enumeration type");

            return Enum.GetValues(typeof(TEnum)).Cast<Enum>().Select((e) => e.Description()).ToList();
        }
    }
}
