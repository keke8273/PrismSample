...
namespace DataLinkLayer.Utils
{
    ...
    /// adapted from <see cref="http://www.sanity-free.com/133/crc_16_ccitt_in_csharp.html"/>
    /// </summary>
    public static class CRC16CCITT
    {
        ...
        /// </summary>
        private const UInt16 _poly = 4129;

        /// <summary>
        /// Initial value
        /// </summary>
        private static UInt16 _initialValue = 0x0000;

        /// <summary>
        /// The lookup table
        /// </summary>
        private static UInt16[] _table = new UInt16[256] 
        {
        ...
        /// <returns>a Uint16 containing the checksum</returns>
        public static UInt16 ComputeChecksum(byte[] bytes, int offset)
        {
            ...
            if ((bytes != null) && (bytes.Length >= 2))
            {
                ...
                {
                    // Break each word into two bytes
                    var lsb = bytes[i];
                    var msb = (i + 1 >= bytes.Length) ? (byte)0xFF : bytes[i + 1];

                    // Perform CRC calculation with MSB first
                    var table_index = ((CRC >> 8) ^ msb) & 0xFF;
                    CRC = (UInt16)((_table[table_index] ^ (UInt16)(CRC << 8)) & 0xFFFF);

                    // Perform CRC calculation with LSB second
                    table_index = ((CRC >> 8) ^ lsb) & 0xFF;
                    CRC = (UInt16)((_table[table_index] ^ (UInt16)(CRC << 8)) & 0xFFFF);
                }
            }
            else
        ...
        {
            return ComputeChecksum(bytes, 0);
        }
