    ...
    public class Win32Usb
    {
        ...
        protected struct HidCaps
        {
            ...
            public override string ToString()
            {
                ...
                builder.Append(string.Format("Usage : 0x{0:X4}\n", Usage));
                builder.Append(string.Format("UsagePage : 0x{0:X4}\n", UsagePage));
                builder.Append(string.Format("InputReportByteLength : 0x{0:X4}\n", InputReportByteLength));
                builder.Append(string.Format("OutputReportByteLength : 0x{0:X4}\n", OutputReportByteLength));
                builder.Append(string.Format("FeatureReportByteLength : 0x{0:X4}\n", FeatureReportByteLength));

                builder.Append(string.Format("Reserved : "));
                foreach (var val in Reserved)
                {
                    builder.Append(string.Format("0x{0:X4} ", val));
                }
                builder.Append(Environment.NewLine);

                builder.Append(string.Format("NumberLinkCollectionNodes : 0x{0:X4}\n", NumberLinkCollectionNodes));
                builder.Append(string.Format("NumberInputButtonCaps : 0x{0:X4}\n", NumberInputButtonCaps));
                builder.Append(string.Format("NumberInputValueCaps : 0x{0:X4}\n", NumberInputValueCaps));
                builder.Append(string.Format("NumberInputDataIndices : 0x{0:X4}\n", NumberInputDataIndices));
                builder.Append(string.Format("NumberOutputButtonCaps : 0x{0:X4}\n", NumberOutputButtonCaps));
                builder.Append(string.Format("NumberOutputValueCaps : 0x{0:X4}\n", NumberOutputValueCaps));
                builder.Append(string.Format("NumberOutputDataIndices : 0x{0:X4}\n", NumberOutputDataIndices));
                builder.Append(string.Format("NumberFeatureButtonCaps : 0x{0:X4}\n", NumberFeatureButtonCaps));
                builder.Append(string.Format("NumberFeatureValueCaps : 0x{0:X4}\n", NumberFeatureValueCaps));
                builder.Append(string.Format("NumberFeatureDataIndices : 0x{0:X4}", NumberFeatureDataIndices));
        ...
        [StructLayout(LayoutKind.Sequential, Pack = 1)]
        public struct HIDAttributesStruct
        {
            public int Size;
        ...
        protected const uint FILE_SHARE_NONE = 0x00;

        protected const uint FILE_SHARE_DELETE = 0x4;
        ...
        protected const int DEVNODES_CHANGED = 0x0007;
        ...
        protected const int SPDRP_DEVICEDESC = 0;

        protected const int SPDRP_DRIVER = 0x9;

        protected const int SPDRP_INSTALL_STATE = 0x22;
        ...
        /// <param name="nDeviceInterfaceDetailDataSize">The size of the above</param>
        /// <param name="nRequiredSize">The required size of the above when above is set as zero</param>
        /// <param name="oDevInfoData">DeviceInfoData to fill with data</param>
        /// <returns></returns>
        [DllImport("setupapi.dll", SetLastError = true)]
        protected static extern bool SetupDiGetDeviceInterfaceDetail(IntPtr lpDeviceInfoSet, ref DeviceInterfaceData oInterfaceData, ref DeviceInterfaceDetailData oDetailData, uint nDeviceInterfaceDetailDataSize, ref uint nRequiredSize, ref DeviceInfoData oDevInfoData);
        ...
        /// <returns>True if successful</returns>
        [DllImport("hid.dll", SetLastError = true)]
        protected static extern bool HidD_GetProductString(IntPtr hFile, IntPtr buff, int len);

        [DllImport("hid.dll", SetLastError = true)]
        protected static extern bool HidD_GetManufacturerString(IntPtr hFile, IntPtr buff, int len);

        [DllImport("hid.dll", SetLastError = true)]
        protected static extern bool HidD_GetSerialNumberString(IntPtr hFile, IntPtr buff, int len);

        [DllImport("hid.dll", SetLastError = true)]
        protected static extern bool HidD_GetAttributes(IntPtr hFile, IntPtr HidDAttributes);
        ...
        protected static extern bool SetupDiGetDeviceRegistryProperty(
            ...
            ref DeviceInfoData iDeviceInfoData,
            uint Property,
